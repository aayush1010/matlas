...
INDUSTRY_TO_MCC_CATEGORY = {
    ...
    "food_delivery": ("5812", "food_and_drink"),
    "auto_repair": ("7538", "transportation"),
    "car_dealer": ("5511", "transportation"),
    "dry_cleaning": ("7211", "general_services"),
    "moving": ("4214", "general_services"),
    "veterinary": ("0742", "general_services"),
}
...
MERCHANTS = [
    ...
    ("DoorDash", "food_delivery", ["doordash", "dd doordash"]),
    ("Grubhub", "food_delivery", ["grubhub"]),
    ("Uber Eats", "food_delivery", ["uber eats", "ubereats"]),
    ("Postmates", "food_delivery", ["postmates"]),
    ("Jimmy John's", "fast_food", ["jimmy johns", "jimmy john's"]),
    ("Jersey Mike's", "fast_food", ["jersey mikes", "jersey mike's"]),
    ("Culver's", "fast_food", ["culvers", "culver's"]),
    ("Sonic Drive-In", "fast_food", ["sonic drive-in", "sonic"]),
    ("Arby's", "fast_food", ["arbys", "arby's"]),
    ("Zaxby's", "fast_food", ["zaxbys", "zaxby's"]),
    ("Raising Cane's", "fast_food", ["raising canes", "raising cane's"]),
    ("H-E-B", "grocery", ["heb", "h-e-b"]),
    ("Wegmans", "grocery", ["wegmans"]),
    ("Food Lion", "grocery", ["food lion"]),
    ("Giant Eagle", "grocery", ["giant eagle"]),
    ("Meijer", "grocery", ["meijer"]),
    ("Winn-Dixie", "grocery", ["winn-dixie", "winn dixie"]),
    ("76", "gas_station", ["76 gas station", "conoco76"]),
    ("Costco Gas", "gas_station", ["costco gas", "costco gasoline"]),
    ("Burlington", "department_store", ["burlington", "burlington coat factory"]),
    ("Ross", "department_store", ["ross", "ross dress for less"]),
    ("TJ Maxx", "department_store", ["tj maxx", "t.j. maxx"]),
    ("Marshalls", "department_store", ["marshalls"]),
    ("Newegg", "electronics", ["newegg"]),
    ("Spirit Airlines", "airline", ["spirit airlines"]),
    ("Frontier Airlines", "airline", ["frontier airlines"]),
    ("Allegiant Air", "airline", ["allegiant air", "allegiant"]),
    ("Hyatt", "hotel", ["hyatt", "hyatt hotels"]),
    ("Holiday Inn Express", "hotel", ["holiday inn express"]),
    ("La Quinta", "hotel", ["la quinta", "la quinta inn"]),
    ("Motel 6", "hotel", ["motel 6"]),
    ("Red Roof Inn", "hotel", ["red roof inn"]),
    ("Extended Stay America", "hotel", ["extended stay america"]),
    ("US Bank", "bank", ["us bank", "u.s. bank"]),
    ("PNC Bank", "bank", ["pnc bank", "pnc"]),
    ("Capital One", "bank", ["capital one"]),
    ("TD Bank", "bank", ["td bank"]),
    ("Ally Bank", "bank", ["ally bank", "ally financial"]),
    ("Allstate", "insurance", ["allstate"]),
    ("Farmers Insurance", "insurance", ["farmers insurance"]),
    ("Liberty Mutual", "insurance", ["liberty mutual"]),
    ("USAA", "insurance", ["usaa"]),
    ("Nationwide", "insurance", ["nationwide", "nationwide insurance"]),
    ("National Grid", "utility", ["national grid"]),
    ("Duke Energy", "utility", ["duke energy"]),
    ("Southern California Edison", "utility", ["socal edison", "southern california edison"]),
    ("Xcel Energy", "utility", ["xcel energy"]),
    ("Alamo Rent A Car", "car_rental", ["alamo", "alamo rent a car"]),
    ("National Car Rental", "car_rental", ["national car rental"]),
    ("Thrifty Car Rental", "car_rental", ["thrifty", "thrifty car rental"]),
    ("Dollar Rent A Car", "car_rental", ["dollar rent a car", "dollar car rental"]),
    ("H&M", "clothing", ["h&m", "h & m"]),
    ("Zara", "clothing", ["zara"]),
    ("Uniqlo", "clothing", ["uniqlo"]),
    ("Forever 21", "clothing", ["forever 21"]),
    ("Express", "clothing", ["express", "express inc"]),
    ("J.Crew", "clothing", ["jcrew", "j.crew"]),
    ("Vans", "shoe", ["vans", "vans store"]),
    ("Journeys", "shoe", ["journeys"]),
    ("Academy Sports + Outdoors", "sporting_goods", ["academy sports", "academy sports + outdoors"]),
    ("REI", "sporting_goods", ["rei", "rei co-op"]),
    ("Big 5 Sporting Goods", "sporting_goods", ["big 5", "big 5 sporting goods"]),
    ("Books-A-Million", "bookstore", ["books-a-million", "books a million"]),
    ("FTD", "florist", ["ftd", "ftd.com"]),
    ("ProFlowers", "florist", ["proflowers"]),
    ("Wawa", "convenience_store", ["wawa"]),
    ("QuikTrip", "convenience_store", ["quiktrip", "qt"]),
    ("Sheetz", "convenience_store", ["sheetz"]),
    ("Casey's General Store", "convenience_store", ["caseys", "casey's general store"]),
    ("Wayfair", "furniture", ["wayfair"]),
    ("Crate & Barrel", "furniture", ["crate & barrel", "crate and barrel"]),
    ("Pottery Barn", "furniture", ["pottery barn"]),
    ("West Elm", "furniture", ["west elm"]),
    ("Advance Auto Parts", "auto_parts", ["advance auto parts"]),
    ("NAPA Auto Parts", "auto_parts", ["napa auto parts", "napa"]),
    ("Firestone Complete Auto Care", "auto_repair", ["firestone", "firestone complete auto care"]),
    ("Midas", "auto_repair", ["midas", "midas auto service"]),
    ("Jiffy Lube", "auto_repair", ["jiffy lube"]),
    ("Pep Boys", "auto_repair", ["pep boys"]),
    ("CarMax", "car_dealer", ["carmax"]),
    ("AutoNation", "car_dealer", ["autonation"]),
    ("1-800-Got-Junk", "moving", ["1-800-got-junk", "1800gotjunk"]),
    ("U-Haul", "moving", ["u-haul", "uhaul"]),
    ("PODS", "moving", ["pods", "pods moving"]),
    ("Tide Cleaners", "dry_cleaning", ["tide cleaners"]),
    ("Zips Dry Cleaners", "dry_cleaning", ["zips dry cleaners", "zips cleaners"]),
    ("Banfield Pet Hospital", "veterinary", ["banfield pet hospital", "banfield"]),
    ("VCA Animal Hospital", "veterinary", ["vca animal hospital", "vca"]),
]
